									...
									<div class="col-xs-12 col-lg-12" style="border: 1px solid #dee2e6!important; padding: 3rem!important">
										<div class="row form-group hr-8">
											<div class="col-lg-6 col-xs-12">
												...
												<select class="form-control" disabled id="transCodeSelect" name="transCodeSelect">
													<option value="">  </option>
													<c:forEach items="${transCom}" var="transComs">
														<option <c:if test="${priceVO.transCode eq transComs.transCode}">selected </c:if>value="${transComs.transCode}">${transComs.transName}</option>
													</c:forEach>
												</select>
												<input type="hidden" name="transCode" id="transCode" value="${priceVO.transCode}"/>
											</div>
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-3 col-xs-12 pd-1rem" >Zone 선택</div>
												<select class="form-control tag-input-style width-100 pd-1rem" disabled id="zoneCodeSelect" name="zoneCodeSelect" data-placeholder="Zone을 선택해 주세요">
													<option value="">  </option>
													<option <c:if test="${priceVO.zoneCode eq 1}">selected </c:if> value="1">1</option>
													<option <c:if test="${priceVO.zoneCode eq 2}">selected </c:if> value="2">2</option>
													<option <c:if test="${priceVO.zoneCode eq 3}">selected </c:if> value="3">3</option>
												</select>
												<input type="hidden" name="zoneCode" id="zoneCode" value="${priceVO.zoneCode}"/>
											</div>
										</div>
										<div class="row form-group hr-8">
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >최소 무게</div>
												<input type="text" name="fromWt" id="fromWt" class="col-lg-12 col-xs-12" value="${priceVO.fromWt}"/>
											</div>
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >최대 무게</div>
												<input type="text" name="toWt" id="toWt" class="col-lg-12 col-xs-12" value="${priceVO.toWt}"/>
											</div>
										</div>
										<div class="row form-group hr-8">
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >최소 금액</div>
												<input type="text" name="minCharge" id="minCharge" class="col-lg-12 col-xs-12" value="${priceVO.minCharge}"/>
											</div>
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >무게 단위</div>
												<input type="text" name="wtUnit" id="wtUnit" class="col-lg-12 col-xs-12" placeholder="미입력시 KG" value="${priceVO.wtUnit}"/>
											</div>
										</div>
										<div class="row form-group hr-8">
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >증가 단위</div>
												<input type="text" name="perWt" id="perWt" class="col-lg-12 col-xs-12" value="${priceVO.perWt}"/>
											</div>
											<div class="col-lg-6 col-xs-12">
												<div class="col-lg-12 col-xs-12 pd-1rem" >단위당 추가 금액</div>
												<input type="text" name="addCharge" id="addCharge" class="col-lg-12 col-xs-12" value="${priceVO.addCharge}"/>
											</div>
										</div>
									</div>
								</div>
