...
<!-- basic scripts -->
	...
	<body class="no-skin">
														...
														<span class="input-group col-md-12" style="display: inline-flex;">
															...
															<select id="transCode" name="transCode" style="height:34px;">
																...
																<c:forEach items="${transCodeList}" var="transCodeList">
																	<option <c:if test="${search.transCode eq transCodeList.transCode}"> selected</c:if> value="${transCodeList.transCode}">${transCodeList.transName}</option>
																</c:forEach>
															</select>
														...
														<c:choose>
															<c:when test="${orgStation eq '082'}">
																<c:choose>
																	...
																	<c:when test="${send.transCode eq 'CJ' || send.transCode eq 'VNP' || send.transCode eq 'YSL' || send.transCode eq 'FB' || send.transCode eq 'FB-EMS' || send.transCode eq 'ACI'}">
		...
		<script type="text/javascript">
			...
			function fn_submit() {
				$("#search-botton").attr('action', '/mngr/send/sendList');
			...
			function fn_excelDown() {
				...
				if (startDate == "" || endDate == "") {
					...
					$("#search-botton").attr('action', '/mngr/send/sendList/excelDown');
					$("#search-botton").attr('method', 'GET');
					$("#search-botton").submit();
				}
			}
