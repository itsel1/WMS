...
<body class="no-skin">
							...
							<div style="width:100%;clear:both;overflow-y:hidden;">
								<div style="width:50%;float:left;padding:10px;">
									...
									<c:forEach items="${itemList}" var="itemList" varStatus="status">
										<div style="width:100%;box-shadow: 0 3px 6px rgba(0, 0, 0, 0.36);margin-bottom:10px;" class="listItem">
											<input type="hidden" name="subNo[]" value="${itemList.subNo}">
											<input type="hidden" name="itemCnt[]" value="${itemList.itemCnt}"> 
											<table id="contents-table">
												<tr>
													<th rowspan="5" style="width:20%;padding-left:10px;">
													...
													<th style="width:10%;">상품 ${itemList.subNo}</th>
													<td style="padding-right:10px;font-size:18px;width:70%:">${itemList.itemDetail}
												...
												<tr>
													<th>제조국</th>
													<td style="padding-right:10px;">${itemList.makeCntry}</td>
												</tr>
												<tr>
													<th>상품단가</th>
													<td style="padding-right:10px;">${itemList.unitValue}
												...
												<tr>
													<th>입고 메모</th>
													<td style="padding-right:10px;"><input type="text" name="whMemo[]" style="width:100%;"></td>
												</tr>
												<tr>
													<th>입고 수량</th>
													<td style="padding-right:10px;">
														<input type="text" class="inputNumber" name="whInCnt[]" onKeyup="this.value=this.value.replace(/[^0-9]/g,'');"> / ${itemList.itemCnt}
													</td>
												</tr>
											</table>
										</div>
								...
								<div style="width:50%;float:right;padding-left:20px;">
									<table id="whInTable">
										...
										<tr>
											<th>단위</th>
											<td>
												<input type="text" name="wta" class="inputWt" onKeyup="this.value=this.value.replace(/[^.0-9]/g,'');" placeholder="wta">
	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			$("#registBtn").one("click",function(e) {
				...
				if (!isChecked) {
					...
					$("input[name='whInCnt[]']").each(function(e) {
						if ($(this).val() == '') {
							alert("입고 수량을 입력해 주세요");
							$(this).focus();
							return;
						} else if ($(this).val() < $('input[name="itemCnt[]"]').get(e).value) {
							alert("입고 수량이 상품 개수보다 작습니다.");
							$(this).focus();
							return;
						} else if ($(this).val() > $('input[name="itemCnt[]"]').get(e).value) {
							alert("입고 수량이 상품 개수보다 많습니다.");
							$(this).focus();
							return;
						}

						var subNoValue = $("input[name='subNo[]']").eq(e).val();
						var whMemoValue = $("input[name='whMemo[]']").eq(e).val();

						whInCntValues.push($(this).val());
						subNoValues.push(subNoValue);
						whMemoValues.push(whMemoValue);
					})
					...
					for (var i = 0; i < subNoValues.length; i++) {
						formData.append('subNoList', subNoValues[i]);
					}
					...
					for (var i = 0; i < whMemoValues.length; i++) {
						formData.append('whMemoList', whMemoValues[i]);
					}
					...
					if ($("input[name='wta']").val() == '') {
						$("input[name='wta']").val("0")
					}
					formData.append('wta', $("input[name='wta']").val());
					formData.append('userWidth', $("input[name='width']").val());
					formData.append('userHeight', $("input[name='height']").val());
					formData.append('userLength', $("input[name='length']").val());
					...
					$.ajax({
						...
						success : function(data) {
							console.log(data);
							if (data.STATUS == 'S10') {
								location.href = "/mngr/return/whInResult?nno="+data.NNO;
							} else {
