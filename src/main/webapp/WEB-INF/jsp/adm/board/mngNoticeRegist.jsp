...
<html lang="en">
	...
	<body class="no-skin">
		...
		<script type="text/javascript">
		var imagename;
		var urls = location.protocol+"//"+location.host+"/";
			    ...
			    addImageBlobHook: function(blob, callback,){
					...
					$.ajax({
			            ...
			            error : function(xhr, status) {
			                alert(xhr + " : " + status);
			                alert("수정sss에 실패 하였습니다. 수정사항을 확인해 주세요.");
			            }
					}) 
		...
		$("#registNotice").on("click",function(){
			$.ajax({
				url:'/mngr/board/registToastNotice',
				type: 'POST',
				data: {
					toastData:editor.getHtml(),
					title:$("#title").val(),
					wDate:$("#datepicker").val()
				} ,
				beforeSend : function(xhr)
				{   /*데이터를 전송하기 전에 헤더에 csrf값을 설정한다*/
				    xhr.setRequestHeader("${_csrf.headerName}", "${_csrf.token}");
