...
<body class="no-skin">
				...
				<div class="page-content">
					...
					<div id="inner-content-side">
						<form name="whoutForm" id="whoutForm" enctype="multipart/form-data">
							...
							<input type="hidden" id="idx" value="${contents.idx}">
							<input type="hidden" id="cIdx" value="${contents.cIdx}">
							<input type="hidden" id="depth" value="${contents.depth}">
							<input type="hidden" id="indent" value="${contents.indent}">
							<div style="width:60%;">
								<div id="">
									<div class="flex-div">
										...
										<div class="element">
											<select id="category" name="category">
												...
												<option value="A" <c:if test="${contents.category eq 'A'}">selected</c:if>>출하지시</option>
												<option value="B" <c:if test="${contents.category eq 'B'}">selected</c:if>>입고안내</option>
											</select>
										</div>
										<div class="element">제목</div>
										<div class="element">
											<input type="text" name="title" id="title" value="${contents.title}">
										</div>
									</div>		
								</div>
									...
									<div id="fileList">
										<c:forEach items="${fileList}" var="fileList" varStatus="status">
											<p id="file${fileList.idx}" class="fileList">
												...
												<c:if test="${userId eq contents.userId}">
													<button data-index="file${fileList.idx}" class="file-del" onclick="fn_delFile('${fileList.idx}', '${fileList.contentId}')" type="button">삭제</button>
												</c:if>
											</p>
										</c:forEach>
									</div>
	...
	<script type="text/javascript">
		...
		$("#reply-btn").on('click', function() {
			location.href = "/mngr/board/whoutReplyIn?idx="+$("#idx").val();
		})

		function fn_delFile(idx, contentId) {
			$.ajax({
				url : '/mngr/board/whoutDelFile',
				type : 'POST',
				data : {idx: idx, contentId: contentId},
