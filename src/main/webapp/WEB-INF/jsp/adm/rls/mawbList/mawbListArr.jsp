				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">도착지 MAWB List</li>
						</ul><!-- /.breadcrumb -->
					</div>
		...
		<script type="text/javascript">
			...
			$("#scrhBtn").on('click ', function(){
			    ...
			    $(grid_selector).setGridParam({
			        url:'/mngr/rls/mawbLoad3',    //*.json 으로 controller 호출
			        datatype: 'json',
			        mtype : 'get',
			        postData: {
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					url :"/mngr/rls/mawbLoad3",
					mtype : "get",
					/* data: grid_data, */
					datatype: "json",
					height: 400,
					colNames:['', 'MawbNo','도착Station','운송사', '편명','출발일', '출발시간', '도착일','도착시간','운송장 개수',''],
					colModel:[
						...
						{name:'hawbCnt',index:'hawbCnt',  sortable: false, editable: false,
							searchoptions:{
								clearSearch:false}
						},
				...
				function testemp(data,pageNo){
					...
					for(var i = (10*(pageNo-1)); i < max; i++){
						...
						$($(".jqgrow").get(i%10)).children().last().append('&nbsp;&nbsp;<button  style="vertical-align:center" class="label label-danger label-white middle" onclick="javascript:mawbShow(\''+data[i].mawbNo+'\')" id="showMawb'+i+' name="showMawb">상세보기</button>')
						$($(".jqgrow").get(i%10)).children().last().append('&nbsp;&nbsp;<button  style="vertical-align:center; cursor:pointer;" class="label label-warning label-white middle" onclick="javascript:maniShow(\''+data[i].mawbNo+'\')" id="maniMawb'+i+' name="maniMawb">Manifest</button>')
					}
