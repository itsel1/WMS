					...
					<div class="page-content">
							...
							<br>
								...
								<div class="display" >
									<table   id="dynamic-table" class="table table-bordered">
										...
										<tbody>
											...
											<tr>
												...
												<td class="colorBlack" style="text-align:center;width:100px;">
												   <a href="#" onclick="fn_popupInvoiceDetail('${list.userId}','${list.invoiceNo}')" >${list.invoiceNo}</a>
												</td>
												...
												<td class="colorBlack" style="text-align:center;width:100px;">
												    <input class="btn btn-sm btn-primary" type="button" value="확정" onclick="fn_Comfirm('${parameterInfo.orgStation}','${list.userId}','${list.invoiceNo}')">
												    <input class="btn btn-sm btn-danger" type="button" value="취소" onclick="fn_Cancle('${parameterInfo.orgStation}','${list.userId}','${list.invoiceNo}')">
												</td>
											</tr>
											</c:forEach>
										</tbody>
									...
									<form id="invoceStatusChange" action="/mngr/invoice/invoceStatusChange" method="get">
										<input type="hidden" id = "orgStation" name="orgStation" value="">
										<input type="hidden" id = "userId" name="userId" value="">
										<input type="hidden" id = "invoiceNo" name="invoiceNo" value="">
										<input type="hidden" id = "actWork" name="actWork" value="">
									</form>						
								</div>
							</div>
		...
		<script type="text/javascript">
			jQuery(function($) {
				$(document).ready(function() {
					...
					$("#ARComfirmList").toggleClass('active');
			...
			function fn_popupInvoiceDetail(userId,invoiceNo){
				var _width = '1220';
			    var _height = '580';
			    var _left = Math.ceil(( window.screen.width - _width )/2);
			    var _top = 150;
				window.open("/mngr/invoice/popupInvoiceDetail?invoiceNo="+invoiceNo+"&userId="+userId,"Invoice Collect",'width='+ _width +', height='+ _height +', left=' + _left + ', top='+ _top);
			} 
			
			function fn_Comfirm(orgStation,userId,invoiceNo){

				$("#orgStation").val(orgStation);
				$("#userId").val(userId);
				$("#invoiceNo").val(invoiceNo);
				$("#actWork").val('decision');

				var formData = $("#invoceStatusChange").serialize();
				$.ajax({
					url:'/mngr/invoice/invoceStatusChange',
					type: 'GET',
			...
			function fn_Cancle(orgStation,userId,invoiceNo){
				$("#orgStation").val(orgStation);
				$("#userId").val(userId);
				$("#invoiceNo").val(invoiceNo);
				$("#actWork").val('cancle');
