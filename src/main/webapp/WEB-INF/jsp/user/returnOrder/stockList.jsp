...
<style type="text/css">
						...
						<div id="inner-content-side" >
							...
							<table id="search-table">
								<tr>
									...
									<td>
										<select id="whStatus" name="whStatus">
											<option value="WO" <c:if test="${params.whStatus eq 'WO'}"> selected </c:if>>정상입고</option>
											<option value="WF" <c:if test="${params.whStatus eq 'WF'}"> selected </c:if>>검수불합격</option>
										</select>
									</td>
									<td>
							...
							<div class="tbl-return">
								<div class="tbl-header">
									<table cellpadding="0" cellspacing="0" border="0">
										<thead>
											<tr>
												<th style="border-right:1px solid #fff;width:7%">검수상태</th>
												<th style="border-right:1px solid #fff;width:7%;">입고일</th>
												...
												<th style="border-right:1px solid #fff;width:18%;">대표상품명</th>
												<th style="border-right:1px solid #fff;width:5%;">요청수량</th>
												<th style="border-right:1px solid #fff;width:5%;">입고수량</th>
												<th style="border-right:1px solid #fff;width:18%;">메세지</th>
											...
											<c:choose>
												<c:when test="${!empty orderList}">
													<c:forEach items="${orderList}" var="orderList" varStatus="status">
														<tr>
															<td style="width:7%;">${orderList.stateName}
															<c:if test="${orderList.state eq 'C003' && orderList.tmpChk ne 0}">
															/ 출고 작업 중
															</c:if>
															</td>
															<td style="width:7%;">${orderList.whInDate}</td>
															<td style="width:10%;">
																<a href="/cstmr/return/stockListDetail?nno=${orderList.nno}">${orderList.koblNo}</a>
																<%-- <a href="/cstmr/return/updateOrderInfo?nno=${orderList.nno}">${orderList.koblNo}</a> --%>
															</td>
															<td style="width:10%;">
																<a href="/cstmr/return/updateOrderInfo?nno=${orderList.nno}">${orderList.trkNo}</a>
															</td>
															<td style="width:10%;">${orderList.trkCom}</td>
															<td style="width:18%;">${orderList.itemDetail}</td>
															<td style="width:5%;">${orderList.totalItemCnt}</td>
															<td style="width:5%;">${orderList.whinCnt}</td>
															<td style="width:18%;padding:0px;text-align:right;">
															...
															<td style="width:10;">
																<c:if test="${orderList.state eq 'C002'}">
																	<input type="button" id="reqWhout" class="btn btn-xs btn-white" onclick="fn_requestWhout('${orderList.nno}')" value="출고 요청">
																</c:if>
																<c:if test="${orderList.state eq 'C003'}">
																	<input type="button" id="reqWhoutCncl" class="btn btn-xs btn-white" onclick="fn_requestWhoutCancel('${orderList.nno}')" value="출고 취소">
																</c:if>
																<input type="button" id="reqDisp" class="btn btn-xs btn-white" onclick="fn_requestDisposal('${orderList.nno}')" value="폐기 요청">
															</td>
														</tr>
													</c:forEach>
												</c:when>
	...
	<script type="text/javascript">
		...
		// 출고 요청
		function fn_requestWhout(nno) {
			fn_requestReturnState(nno, 'WHOUT');
		}

		// 폐기 요청
		function fn_requestDisposal(nno) {
			fn_requestReturnState(nno, 'DISP');
		}

		function fn_requestWhoutCancel(nno) {
			fn_requestReturnState(nno, 'CNCL');
		}
		
		function fn_requestReturnState(nno, type) {
			$.ajax({
				url : '/cstmr/return/requestReturnState',
				type : 'POST',
				data : {nno: nno, type: type},
				...
				success : function(data) {
					if (data.status == 'S') {
						if (type=='WHOUT') {
							alert("출고 요청 처리 되었습니다.\n출고 작업 시 요청 취소가 불가능합니다.");
						} else if (type=='DISP') {
							alert("폐기 요청 처리 되었습니다.");
						} else {
							alert("출고 취소 처리 되었습니다.");
						}
						location.reload();
					} else if (data.status == 'F') {
						alert("요청 처리에 실패 하였습니다. 다시 시도해 주세요.");
					} else if (data.status == 'N') {
						alert("출고 작업 중인 반품 건입니다.\n관리자에게 문의해 주세요.");
					}
				},
