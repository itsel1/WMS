...
<html>
	...
	<body class="no-skin">
		...
		<div class="toppn">
	      <div class="container">
	        <div class="row">
	          <div class="col-md-12 mb-0" style="font-size:14px !important;"><a>등록내역</a> <span class="mx-2 mb-0">/</span> <strong class="text-black">BL 프린트</strong></div>
	        </div>
	      </div>
	    </div>
	    <!-- Main container Start-->
											...
											<tbody>
												<c:forEach items="${shpngList}" var="shpngAllList" varStatus="status">
													<tr>
														...
														<td class="center colorBlack" >
															<c:if test="${selTransCode ne 'ARA' }"><a href="/cstmr/apply/modifyRegistShpng?types=shpng&nno=${shpngAllList.value[0].nno}">${shpngAllList.value[0].orderNo}</a></c:if>
															<c:if test="${selTransCode eq 'ARA' }">${shpngAllList.value[0].orderNo}</c:if>
														</td>
														<td class="center colorBlack">
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				$('#formSelect').on('change', function(){
					location.href="/cstmr/apply/shpngRegistList?transCode="+$(this).val();
				});
