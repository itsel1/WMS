											...
											<tbody>
												<c:forEach items="${inspList}" var="inspAllList" varStatus="status">
													<tr>
														...
														<td class="center colorBlack" >
															<c:if test="${selTransCode ne 'ARA' }"><a href="/cstmr/apply/modifyRegistInfoInsp?types=insp&nno=${inspAllList.value[0].nno}">${inspAllList.value[0].orderNo}</a></c:if>
															<c:if test="${selTransCode eq 'ARA' }">${inspAllList.value[0].orderNo}</c:if>
				...
				$('#delCheck').on('click',function(e){
					...
					$.ajax({
						...
						success : function(data) {
							if(data == "S"){
								alert("삭제 되었습니다.")
								location.href="/cstmr/apply/inspRegistList";
							}else if(data == "F"){
