	...
	<body>
							...
							<div class="page-header">
								<h3>
									<c:if test="${urlType eq 'allAbout'}">
										${userOrder.orderNo} 검품배송 등록내역 수정
									</c:if>
								...
								<div class="row">
									<div class="col-xs-12 col-lg-12" >
										<div class="col-xs-12 col-lg-12 " style="font-size: 5;">
											<h2>
												주문정보
												<c:if test="${!empty userOrder.shipperReference}">
													(REFERENCE NO : ${userOrder.shipperReference})
												</c:if>
											</h2>
										</div>
										...
										<div class="col-lg-12">
											<div id="button-div">
												<div id="backPage" style="text-align: center">
													<button id="backToList" type="button" class="btn btn-sm btn-danger">뒤로</button>
												</div>
											</div>
										</div>
		...
		<script type="text/javascript">
			...
			$('#backToList').on('click', function(e){
				if($("#urlType").val() == 'allAbout')
					location.href = "/cstmr/apply/inspRegistList?transCode="+$("#transCode").val()
				else if($("#urlType").val() == 'before')
					location.href = "/cstmr/apply/inspSendBefore?transCode="+$("#transCode").val()
				else if($("#urlType").val() == 'after')
					location.href = "/cstmr/apply/inspSendAfter?transCode="+$("#transCode").val()
