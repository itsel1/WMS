					...
					<div id="inner-content-side">
						<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
							...
							<div id="table-contents" class="table-contents" style="width:100%; overflow: auto;">
								...
								<div class="display" style="width:2000px;">
											...
											<c:forEach items="${shpngList}" var="shpngAllList" varStatus="status">
												<tr>
													...
													<td class="center colorBlack">
														...
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP09')}">
															<span class="red errorCol"> 이미 등록된 주문번호 입니다. <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP01')}">
															<span class="red errorCol"> 주문번호가 너무 깁니다.<br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP02')}">
															<span class="red errorCol"> 송하인 우편번호가 너무 깁니다.(최대 8자리) <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP03')}">
															<span class="red errorCol"> 송하인 전화번호 양식을 확인해 주세요 <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP04')}">
															<span class="red errorCol"> 송하인 핸드폰번호 양식을 확인해 주세요 <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP05')}">
															<span class="red errorCol"> 수취인 연락처1을 확인해 주세요 <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP06')}">
															<span class="red errorCol"> 수취인 연락처2을 확인해 주세요 <br /></span>
														</c:if>
														<c:if test="${fn:contains(shpngAllList.value[0].status,'TMP07')}">
															<span class="red errorCol"> 수취인 우편주소 양식을 확인해 주세요.(최대 8자리)<br /></span>
														</c:if> 
														<c:if test="${!empty shpngAllList.value[0].errName}">
															<c:if test="${shpngAllList.value[0].status ne 'A'}">
																<span class="red errorCol">${shpngAllList.value[0].errName}</span>
															</c:if> 
														</c:if>
														...
														<c:if test="${empty shpngAllList.value[0].orgStationName}">
															<span class="red errorCol"> 
																출발지를 확인해 주세요.<br />
															</span>
														</c:if>
														<c:if test="${empty shpngAllList.value[0].dstnNationName}">
															<span class="red errorCol"> 
																도착지를 확인해 주세요.<br />
															</span>
														</c:if>
														...
														<c:forEach items="${shpngAllList.value}" var="shpngList" varStatus="status">
															...
															<c:if test="${empty shpngList.makeCntry}">
	...
	<script type="text/javascript">
			jQuery(function($) {
				$(document).ready(function() {
					...
					jQuery('.errorCol').parent().parent().css('background-color', 'rgb(255,242,242)');
				...
				$("#dstnNation").on('change',function(e){
					location.href = '/cstmr/apply/shpngAgncy?transCode='+$("#dstnNation").val();
				})
				...
				$('#excelUp').on('click', function(e) {
					...
					$.ajax({
						...
						url : "/cstmr/apply/shpngAgncyExcelUpload?formSelect="+$("#dstnNation").val(),
						data : datass,
						processData: false,
			            contentType: false,
						error : function(){
