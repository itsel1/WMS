							...
							<div class="col-xs-12 form-group" style="margin:0px;">
								<div class="row">
									<div class="col-xs-12 col-md-2 form-group">
										<select id="orgNation" name="orgNation" style="height: 34px; width: 100%;">
											<!-- <option value="0">선택하세요</option>
													<option value="SAGAWA">SAGAWS</option> -->
											<c:forEach items="${nationInfo}" var="orgNation" varStatus="status">
												<c:if test="${nationInfo[status.index].orgNation ne nationInfo[status.count].orgNation}">
													<option value="${orgNation.orgNation}">${orgNation.orgName}</option>
												</c:if>
											</c:forEach>
										</select>
									</div>
			...
			jQuery(function($) {
				...
				var myTable = 
					...
					$("#testBtn").on('click',function(e){
						window.open("/cstmr/apply/excelFormDown?trans="+$("#dstnNation").val())
					})
			})
		</script>
	<!-- script addon end -->
