...
<head>
	...
	<body class="no-skin"> 
		...
		<!-- Main container Start-->
			<div class="toppn" style="">
				<div class="container">
					<div class="row">
						<div class="col-md-12 mb-0" style="font-size:14px !important;">
							<a>반품 페이지</a> <span class="mx-2 mb-0">/</span> <strong class="text-black">반품 검수 내역</strong>
						</div>
		        	</div>
		      	</div>
		    </div>
			...
			<div class="container">
		       <div class="page-content noneArea">
					<div id="inner-content-side" >
						<br/>
						<h2 style="margin-left:4px;">반품 검수 내역</h2>
						<br/><br/>
						<div style="margin-left:20px; margin-bottom:10px; width:100%; vertical-align:middle;">
							<select id="inspType" name="inspType" style="height:34px; width:100px;">
								...
								<option <c:if test="${params.inspType == 'C003'}"> selected</c:if> value="C003">출고대기</option>
								<option <c:if test="${params.inspType == 'C004'}"> selected</c:if> value="C004">출고승인</option>
								<option <c:if test="${params.inspType == 'D004'}"> selected</c:if> value="D004">불합격</option>
							</select>
								...
								<table style="width:100%;">
									<thead style="border-bottom:2px double #ccc;">
										<tr>
											...
											<th style="width:10%; text-align:center; border-right:1px dotted #ccc;">검수 상태</th>
											...
											<th style="width:15%; text-align:center; border-right:1px dotted #ccc;">등록일</th>
											<th style="width:10%; text-align:center; border-right:1px dotted #ccc;">반송 택배사</th>
											<th style="width:15%; text-align:center; border-right:1px dotted #ccc;">반송 송장번호</th>
											<th style="width:13%; text-align:center; border-right:1px dotted #ccc;">반송 접수일</th>
												...
												<c:forEach items="${inspList}" var="list" varStatus="status">
													<tr>
														...
														<td style="text-align:center; height:40px; border-top:1px solid #ccc; border-right:1px dotted #ccc;">
															...
															<c:if test="${list.STATE eq 'C003'}">
																출고 대기&nbsp;&nbsp;&nbsp;<input type="button" class="addBtn" onclick="fnAcceptReturn('${list.ORDER_REFERENCE}')" value="승인"/>
															</c:if>
															<c:if test="${list.STATE eq 'D004'}">
																불합격
															</c:if>
															<c:if test="${list.STATE eq 'C004'}">
																출고 승인&nbsp;&nbsp;&nbsp;<input type="button" class="addBtn" onclick="fnUnAcceptReturn('${list.ORDER_REFERENCE}')" value="대기"/>
															</c:if>
														</td>
														<td style="text-align:center; height:40px; border-top:1px solid #ccc; border-right:1px dotted #ccc;">
