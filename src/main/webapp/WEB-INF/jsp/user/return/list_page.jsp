								...
								<div class="col-xs-12 col-sm-11 col-md-10">
									<span class="input-group" style="display: inline-flex; width:100%">
										<select name="option" style="height: 34px">
											...
											<option <c:if test="${option eq 'hawbNo'}"> selected </c:if>value="hawbNo">국내송장번호</option>
										</select>
						...
						<table id="dynamic-table" class="table table-bordered containerTable">
							<thead>
								<tr height="30" >
									...
									<th rowspan="2" class="center">진행상황</th>
									<th rowspan="2" class="center">원운송장번호</th>
									<th rowspan="2" class="center">ORDER NO</th>
									<th rowspan="2" class="center">셀러 ID</th>
									<th rowspan="2" class="center">반송유형</th>
									<th rowspan="2" class="center">접수일</th>
								...
								<c:forEach items="${vo}" var="test" varStatus="status">
									<tr>
										...
										<td class="center colorBlack" style="word-break:break-all;">
											<c:choose>
												<c:when test="${test.STATE eq 'A001' or test.STATE eq 'A000' or test.STATE eq 'A002' or test.STATE eq 'B001'}">
													<c:if test="${!empty test.KOBL_NO}">
														<a href='/return/info/${test.orderReference}'>${test.KOBL_NO}</a>
													</c:if>
													<c:if test="${empty test.KOBL_NO}">
												...
												<c:when test="${test.STATE eq 'C003' or test.STATE eq 'C004' or test.STATE eq 'C002'}">
													<c:if test="${!empty test.KOBL_NO}">
														<a href='/return/return/C002/${test.ORDER_REFERENCE}'>${test.KOBL_NO}</a>
													</c:if>
													<c:if test="${empty test.KOBL_NO}">
														<a href='/return/return/C002/${test.ORDER_REFERENCE}'>-</a>
													</c:if>
												</c:when>
												<c:when test="${test.STATE eq 'D005' or test.STATE eq 'D004'}">
													<c:if test="${!empty test.KOBL_NO}">
														<a href='/return/return/D005/${test.ORDER_REFERENCE}'>${test.KOBL_NO}</a>
													</c:if>
													<c:if test="${empty test.KOBL_NO}">
														<a href='/return/return/D005/${test.ORDER_REFERENCE}'>${test.KOBL_NO}</a>
													</c:if>
										...
										<td class="center colorBlack" style="word-break:break-all;">
											<c:choose>
												<c:when test="${test.STATE eq 'A001' or test.STATE eq 'A000' or test.STATE eq 'A002' or test.STATE eq 'B001'}">
													<c:if test="${!empty test.ORDER_NO}">
														<a href='/return/info/${test.ORDER_REFERENCE}'>${test.ORDER_NO}</a>
													</c:if>
													<c:if test="${empty test.ORDER_NO}">
														<a href='/return/info/${test.ORDER_REFERENCE}'>-</a>
													</c:if>
												</c:when>
												<c:when test="${test.STATE eq 'C003' or test.STATE eq 'C004' or test.STATE eq 'C002'}">
													<c:if test="${!empty test.ORDER_NO}">
														<a href='/return/return/C002/${test.ORDER_REFERENCE}'>${test.ORDER_NO}</a>
													</c:if>
													<c:if test="${empty test.ORDER_NO}">
														<a href='/return/return/C002/${test.ORDER_REFERENCE}'>-</a>
													</c:if>
												...
												<c:when test="${test.STATE eq 'D005' or test.STATE eq 'D004'}">
													<c:if test="${!empty test.ORDER_NO}">
														<a href='/return/return/D005/${test.ORDER_REFERENCE}'>${test.ORDER_NO}</a>
													</c:if>
													<c:if test="${empty test.ORDER_NO}">
														<a href='/return/return/D005/${test.ORDER_REFERENCE}'>${test.ORDER_NO}</a>
													</c:if>
										...
										<td class="center colorBlack" style="word-break:break-all;">
											<c:if test="${test.RETURN_TYPE eq 'E'}">긴급배송</c:if>
											<c:if test="${test.RETURN_TYPE eq 'N'}">일반배송</c:if>
										</td>
										<td class="center colorBlack" style="word-break:break-all;">
											${test.REGIST_DATE}	<!-- 등록일 -->
										</td>
										<td class="center colorBlack" style="word-break:break-all;">
											${test.RE_TRK_COM}<!-- 수거택배사 -->
										</td>
										<td class="center colorBlack" style="word-break:break-all;">
											${test.RE_TRK_NO}<!-- 수거택배번호 -->
										</td> 
										<td class="center colorBlack" style="word-break:break-all;">
											${test.RE_TRK_DATE}<!-- 수거접수일 -->
										</td> 
										<td class="center colorBlack" style="word-break:break-all;">
											${test.TRANS_CODE}<!-- 발송택배사 -->
										</td> 
										<td class="center colorBlack" style="word-break:break-all;">
											<!--${test.transWDate} 발송택배번호 -->
											${test.HAWB_NO}
										</td> 
										<td class="center" style="overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">
											${test.USER_MSG}
										</td> 
										<td class="center" style="overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">
											${test.ADMIN_MSG}
										...
										<td class="center colorBlack">
											<label style="cursor:pointer; color:#46649B;" onclick="fn_popupMsg('${test.NNO}', '${test.ORG_STATION}', '${test.SELLER_ID}')">등록</label>
										</td> 
										<td style="text-align: center;">
											<input type="hidden" id="acceptType" name="acceptType" value=""/>
											<c:if test="${test.PICK_TYPE eq 'A' and test.STATE eq 'A002'}">
												<input type="button" id="updateStatus" name="updateStatus" onclick="fnUpdateStatus('${test.ORDER_REFERENCE}')" value="발송">
											</c:if>
											<c:if test="${test.state eq 'C003' }">
												<input type="button" id="acceptReturn" name="acceptReturn" onclick="fnAcceptReturn('${test.ORDER_REFERENCE}')" value="승인">
											</c:if>
											<c:if test="${test.state eq 'C004' }">
												<input type="button" id="unAcceptReturn" name="unAcceptReturn" onclick="fnUnAcceptReturn('${test.ORDER_REFERENCE}')" value="대기">
											</c:if>
										</td>
									</tr>
								</c:forEach>
...
*/
		...
		function fn_popupMsg(nno, orgStation, userId) {
			window.open("/return/popupMsg?nno="+nno+"&orgStation="+orgStation+"&userId="+userId, "PopupWin", "width=1200, height=700");
		}
